...
const fetchUserContextTextFromGas = async () => {
  ...
  try {
    ...
    if (!response.ok) {
      setStatus('店舗情報の取得に失敗しました。時間をおいてお試しください。', 'error')
      return { status: 'error', text: '' }
    }
    ...
    setStatus('店舗情報の取得に失敗しました。ネットワーク状況をご確認ください。', 'error')
    return { status: 'error', text: '' }
  }
}
